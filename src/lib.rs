    ...
    T: Fixed,
{
    ...
    // Updated uncertainty
    let new_uncertainty =
        state.process_noise * state.uncertainty * (T::TRY_ONE.unwrap() - kalman_gain);
...
mod tests {
    ...
    fn test_with_monotonic_increasing_f64() {
        ...
        for i in 1..=MAX_ITERATIONS {
            let kalman_gain =
                kstate.uncertainty / (kstate.uncertainty + kstate.measurement_uncertainty);
            println!(
                "gain {} uncert: {} munc: {}",
                kalman_gain, kstate.uncertainty, kstate.measurement_uncertainty
            );
